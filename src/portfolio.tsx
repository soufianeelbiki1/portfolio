...
const retailEvidence = `${github}RetailIntel/blob/878526db1753ecca415f4f06dc717e8d90a1f9b9/docs/DECISION_WALKTHROUGH.md`;
const nexusLive = 'https://nexus-soufiane15.vercel.app/';
const scenarios = {
...
const snapshots = [
  ...
  ['retailintel','RetailIntel','Inventory decisions','Synthetic demand, safety stock, policy inputs and holdout evidence.'],
  ...
  return <div className="folio" data-reduced-motion={reduced ? 'true' : 'false'}>
    ...
    <main id="main">
      ...
      <section className="folio-wrap selected-work" aria-labelledby="work-heading">
        ...
        <article className="work-row" id="payments"><div className="work-number mono">01</div><div className="work-main"><span className="work-type mono">PAYMENTS & OPERATIONS</span><h3>AtlasPay <span>+ Nexus</span></h3><p>Preserve payment intent through retries and make failure states visible to an operator.</p><details><summary>Inside the design <span aria-hidden="true">+</span></summary><div className="decision-text"><p>ISO 8583 simulation, double-entry accounting and a transactional outbox. The Java authorization work verifies concurrent retries and rollback against real PostgreSQL; the Python-to-Nexus demo checks unavailable and recovery states.</p><p className="review-note">Nexus #26 is merged and its live read-only dashboard is browser-verified. AtlasPay #38/#39 remains a reviewed branch, not a deployed Java release. The browser model above is illustrative.</p><a href={evidence}>Pinned tests & reproduction <Arrow diagonal/></a></div></details><div className="work-links"><a href={`${github}AtlasPay`}>AtlasPay source <Arrow diagonal/></a><a href={`${github}Nexus`}>Nexus source <Arrow diagonal/></a><a href={nexusLive}>Live Nexus <Arrow diagonal/></a></div></div><ArchitectureMark variant="payments"/></article>
        <article className="work-row" id="retail"><div className="work-number mono">02</div><div className="work-main"><span className="work-type mono">DATA & BUSINESS DECISIONS</span><h3>RetailIntel</h3><p>An inventory queue that makes forecast error and purchasing assumptions part of the decision.</p><details><summary>Inside the design <span aria-hidden="true">+</span></summary><div className="decision-text"><p>DuckDB transformations use the scored prior-only mean in the replenishment policy, then place each SKU’s MAE, WAPE and sample count beside the proposed order. Point-in-time guards prevent post-snapshot evidence from supporting an earlier decision; the generated dashboard also prints its seed, sample size and decision cutoffs.</p><p className="review-note">Synthetic data. Forecast evaluation, visible provenance and accessible table navigation are merged in RetailIntel #6. No real-world savings or optimal inventory claim.</p><a href={retailEvidence}>Pinned tests & reproduction <Arrow diagonal/></a></div></details><div className="work-links"><a href={`${github}RetailIntel`}>RetailIntel source <Arrow diagonal/></a><a href="/demos/retailintel.html">Report snapshot <Arrow/></a></div></div><ArchitectureMark variant="retail"/></article>
