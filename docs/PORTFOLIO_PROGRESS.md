...
Nexus #26 now has head `1a92bdc8b0e6dbec954eb46f759e2482bc74adb2`: demo CI pins AtlasPay `7990d04f2485b9cf46ab5c540b0418a128b48a7b`, covers lockfile-triggered runs, checks authenticated server-rendered output, shuts down the local API to assert unavailable output with no fixture/live-data fallback, then restarts it and checks recovery without reseeding. Local YAML parsing, all workflow Bash syntax checks and diff whitespace checks pass. All six checks on the prior head succeeded, including the integrated demo; these do not validate the new head. New-head remote checks and public browser verification must be completed before merge. The local demo guide records the pinned baseline and the limits of HTTP assertions. Next priority: inspect the new-head outage/recovery CI result, then verify production deployment side effects before merging.
