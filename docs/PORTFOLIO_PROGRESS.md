...
## Java authorization replay correctness (2026-09-12)

[AtlasPay ready-for-review #38](https://github.com/soufianeelbiki1/AtlasPay/pull/38), head `a3f67692149156d1cfc78cc24e58e39e0f3f44f7`, compares persisted payment/issuer/amount/currency on retry. Changed requests now conflict with HTTP 409; identical retries retain the original decision and perform no writes. Added service-unit regression cases for all four changed fields and identical replay; docs explicitly note that these are mocks, not PostgreSQL concurrency proof. Local diff whitespace validation passed. Local Java is 17 and Maven is absent. Java 21 CI now reports BUILD SUCCESS: seven tests, zero failures/errors/skips, including three authorization service tests (one loops over four mismatched fields). Root test and container checks also pass. Marked ready after reviewing the diff and CI evidence; no merge. No main merge, hosted database calls or cloud provisioning. Concurrent first-request race handling remains follow-up work; prioritize a PostgreSQL integration test after this bounded fix. Prior ready AtlasPay #37 and Nexus #26 heads/checks were rechecked unchanged; #25 remains blocked until #26 can safely merge.
